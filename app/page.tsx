...
const ui: Record<string, Bi> = {
  verified: ["ตรวจสอบสูตรแล้ว", "Formula verified"], install: ["ดาวน์โหลดแอป", "Download App"], update: ["อัปเดตแอป", "Update App"], updating: ["กำลังอัปเดต…", "Updating…"], inputs: ["ข้อมูลนำเข้า", "INPUTS"], reset: ["ค่าเริ่มต้น", "Reset"], results: ["ผลการคำนวณ", "CALCULATION RESULTS"], formula: ["สูตรที่ใช้", "Formula used"], note: ["รายงาน PDF จะรวมข้อมูลนำเข้า ผลลัพธ์ และสูตรนี้", "The PDF report includes inputs, results, and this formula."], export: ["ส่งออก PDF", "Export PDF"], copy: ["คัดลอกผลลัพธ์", "Copy results"], copied: ["คัดลอกแล้ว ✓", "Copied ✓"], report: ["รายงานการคำนวณ CalFlow", "CalFlow calculation report"],
};
...
function ScientificCalculator({ lang, exportLabel }: { lang: Lang; exportLabel: string }) {
  const [expression, setExpression] = useState("sin(30)"), [angleMode, setAngleMode] = useState<AngleMode>("DEG"), [ans, setAns] = useState(.5), [memory, setMemory] = useState(0), [history, setHistory] = useState<Array<{ expression: string; result: number }>>([]), [copied, setCopied] = useState(false), [shift, setShift] = useState(false), [hyperbolic, setHyperbolic] = useState(false), [fractionMode, setFractionMode] = useState(false), [fractionBuilderOpen, setFractionBuilderOpen] = useState(false), [fractionNumerator, setFractionNumerator] = useState(""), [fractionDenominator, setFractionDenominator] = useState("");
  ...
  const fractionPreview = useMemo(() => { try { const numerator = evaluateExpression(fractionNumerator, angleMode, ans), denominator = evaluateExpression(fractionDenominator, angleMode, ans); if (denominator === 0) throw new Error("Division by zero"); return { value: numerator / denominator, valid: true }; } catch { return { value: NaN, valid: false }; } }, [fractionNumerator, fractionDenominator, angleMode, ans]);
  ...
  const functions = [...trigFunctions, ["ln", "ln("], ["log₁₀", "log("], ["√", "sqrt("], ["∛", "cbrt("], ["x²", "square"], ["xʸ", "^"], ["1/x", "reciprocal"], ["x!", "!"], ["nCr", " nCr "], ["nPr", " nPr "], ["fraction", "fractionTemplate"], ["|x|", "abs("], ["eˣ", "exp("], ["⌊x⌋", "floor("], ["⌈x⌉", "ceil("]] as const;
  ...
  const press = (action: string) => {
    ...
    else if (action === "fractionTemplate") { setFractionNumerator(expression === "sin(30)" ? "" : expression); setFractionDenominator(""); setFractionBuilderOpen(true); }
    else append(action);
  };
  const useFraction = () => {
    if (!fractionPreview.valid) return;
    setExpression(`(${fractionNumerator})/(${fractionDenominator})`);
    setFractionBuilderOpen(false);
  };
  return <section className="scientific-section" aria-live="polite">
    ...
    <div className="scientific-layout">
      <div className="calculator-shell glass-card">
        <div className="calculator-device-head"><div><strong>CalFlow</strong><span>ENGINEERING</span></div><p>CF-991 <b>PRO</b></p></div>
        ...
        {fractionBuilderOpen && <div className="fraction-composer no-print"><div><span>{lang === "th" ? "สร้างเศษส่วน" : "FRACTION TEMPLATE"}</span><strong>{fractionPreview.valid ? `= ${formatCalculatorValue(fractionPreview.value)}` : "a ⁄ b"}</strong></div><div className="fraction-stack"><input autoFocus aria-label={lang === "th" ? "ตัวเศษ" : "Numerator"} value={fractionNumerator} onChange={(event) => setFractionNumerator(event.target.value)} placeholder={lang === "th" ? "ตัวเศษ" : "Numerator"}/><i/><input aria-label={lang === "th" ? "ตัวส่วน" : "Denominator"} value={fractionDenominator} onChange={(event) => setFractionDenominator(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") useFraction(); }} placeholder={lang === "th" ? "ตัวส่วน" : "Denominator"}/></div><div className="fraction-actions"><button onClick={() => setFractionBuilderOpen(false)}>{lang === "th" ? "ยกเลิก" : "Cancel"}</button><button disabled={!fractionPreview.valid} onClick={useFraction}>{lang === "th" ? "ใช้ในสมการ" : "Use fraction"}</button></div></div>}
        ...
        <div className="calculator-navigation no-print" aria-label={lang === "th" ? "แผงควบคุม" : "Navigation pad"}><button onClick={() => setExpression((current) => current.slice(0, -1))}>◀</button><button onClick={() => history[0] && setExpression(history[0].expression)}>▲</button><button className="nav-ok" onClick={calculateNow}>OK</button><button onClick={() => history[1] && setExpression(history[1].expression)}>▼</button><button onClick={() => append("Ans")}>▶</button></div>
        ...
        <div className="calculator-key-area"><div className="function-grid no-print">{functions.map(([label, action]) => <button key={label} aria-label={action === "fractionTemplate" ? (lang === "th" ? "ป้อนเศษส่วนสองบรรทัด" : "Enter stacked fraction") : label} onClick={() => { press(action); if (shift) setShift(false); }}>{action === "fractionTemplate" ? <span className="fraction-key"><i>□</i><i>□</i></span> : label}</button>)}</div><div className="calculator-keypad no-print">{keypad.map(([label, action]) => <button key={label} className={action === "equals" ? "equals" : ["÷", "×", "−", "+", "^"].includes(action) ? "operator" : action === "clear" || action === "back" ? "utility" : ""} onClick={() => press(action)}>{label}</button>)}</div></div>
