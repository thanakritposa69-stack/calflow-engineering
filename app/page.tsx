  ...
  return <section className="scientific-section" aria-live="polite">
    ...
    <div className="scientific-layout">
      <div className="calculator-shell glass-card">
        ...
        <div className="function-grid no-print">{functions.map(([label, action]) => <button key={label} aria-label={action === "fractionTemplate" ? (lang === "th" ? "ป้อนเศษส่วนสองบรรทัด" : "Enter stacked fraction") : label} onClick={() => { press(action); if (shift) setShift(false); }}>{action === "fractionTemplate" ? <span className="fraction-key"><i>□</i><i>□</i></span> : label}</button>)}</div>
