...
test("server-renders the CalFlow calculator", async () => {
  ...
  assert.match(html, /topbar-credit/);
  assert.doesNotMatch(html, /\bREV\s*\d+/i);
...
test("includes a safe full scientific calculator", async () => {
  ...
  assert.match(source, /display-input/);
  assert.match(source, /display-output/);
  assert.match(source, /formatFraction/);
  assert.match(source, /nCr/);
  assert.match(source, /nPr/);
  assert.match(source, /SHIFT/);
  assert.match(source, /calculator-device-head/);
  assert.match(source, /calculator-navigation/);
  assert.match(source, /fractionTemplate/);
